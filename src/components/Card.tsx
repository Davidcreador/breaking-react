...
const Card = ({ episode }: { episode: EpisodeType }): JSX.Element => {
  return (
    <div className="max-w-sm bg-white shadow-lg rounded-lg overflow-hidden my-4">
      <div className="flex items-center px-3 py-3 bg-gray-900">
        <h2 className="mx-3 text-white font-semibold text-lg">Season: {episode.season}</h2>
      </div>
      <div className="py-4 px-6">
        <h1 className="text-2xl font-semibold text-gray-800">Title: {episode.title}</h1>
        <p className="py-2 text-lg text-gray-700">Episode: {episode.episode}</p>
        {episode.characters.map((character, index) => (
          <div key={index} className="flex items-center mt-4 text-gray-700">
            ...
            <h3 className="px-2 text-sm">{character}</h3>
          </div>
        ))}
        <div className="flex items-center mt-4 text-gray-700">
          ...
          <h1 className="px-2 text-sm">Date: {episode.air_date}</h1>
        </div>
      </div>
    </div>
  )
